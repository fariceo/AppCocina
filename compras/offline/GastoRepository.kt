...
class GastoRepository(context: Context) {
    ...
    fun actualizarEstadoOffline(id: Int, nuevoEstado: Int) {
        ...
        Log.d("GastoRepo", "Estado actualizado offline: $rowsUpdated filas para id=$id")
        db.close()
    }
    fun actualizarPrecioOffline(id: Int, nuevoPrecio: Double) {
        ...
        val rows = db.update("gastos", values, "id = ?", arrayOf(id.toString()))
        Log.d("GastoRepo", "Nuevo precio guardado: $nuevoPrecio para id=$id")
        db.close()
    }
