...
class GastosActivity : AppCompatActivity() {
    ...
    private fun cargarGastosConFallback(filtro: String) {
        if (hayConexionInternet()) {
            ...
            listaGastos.clear()
            listaGastos.addAll(gastosOffline.map { Gasto(it.id, it.producto, it.total, it.estado) })
            ...
            { response ->
                try {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        Log.d("SYNC_SERVER", "ID: $id | Producto: $producto | Total: $total | Estado: $estado | Fecha: $fecha | Hora: $hora") // <-- NUEVO
                        offlineList.add(
                            GastoOffline(
                                id = id,
                                producto = producto,
    ...
    private fun configurarAdapterYVista(
        ...
        gastosAdapter.onEditarPrecio = { gasto ->
            if (gasto.estado == 1) {
                ...
                et.setText(gasto.precio.toString())

                AlertDialog.Builder(this)
                    .setTitle("Editar precio de ${gasto.producto}")
                    .setView(et)
                    .setPositiveButton("Guardar") { _, _ ->
                        et.text.toString().toDoubleOrNull()?.let { nuevo ->
                            if (hayConexionInternet()) {
                                actualizarPrecioServidor(gasto.id, nuevo, gasto.estado) {
                                    Toast.makeText(this, "Precio actualizado online", Toast.LENGTH_SHORT).show()
                                    cargarGastosConFallback(currentFiltro)  // refrescar lista online
                                }
                            } else {
                                gastoRepo.actualizarPrecioOffline(gasto.id, nuevo)
                                Toast.makeText(this, "Precio actualizado offline", Toast.LENGTH_SHORT).show()
                                cargarGastosConFallback(currentFiltro)  // refrescar lista offline
                                sincronizarDatosOffline()
                            }
                        }
                    }
        ...
        gastoRepo.guardarGastos(
            listOf(
                GastoOffline(
                    ...
                    peso = "",
                    total = precio,
                    fecha = fechaActual,  // <-- Guardar fecha real
                    hora = horaActual,    // <-- Guardar hora real
                    estado = 0,
                    sincronizado = false
                )
            )
    ...
    private fun actualizarPrecioServidor(id: Int, nuevo: Double, estado: Int = 0, onSuccess: () -> Unit) {
        val url = "https://elpollovolantuso.com/asi_sistema/android/actualizar_estado_gasto.php"
        val rq = Volley.newRequestQueue(this)
        val req = object : StringRequest(Request.Method.POST, url,
            { resp ->
                if (JSONObject(resp).optBoolean("success", false)) {
                    onSuccess()
                } else {
                    Toast.makeText(this, "No se pudo actualizar en el servidor", Toast.LENGTH_SHORT).show()
                }
            },
            { _ -> Toast.makeText(this, "Error de red", Toast.LENGTH_SHORT).show() }
        ) {
            override fun getParams() = mapOf(
                "id" to id.toString(),
                ...
                { resp ->
                    try {
                        ...
                        if (json.optBoolean("success", false)) {
                            ...
                            cargarGastosConFallback(currentFiltro) // refrescar vista
                        }
                    } catch (e: Exception) {
